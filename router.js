module.exports = (function () {
    ...
    function router(request, response) {
        if (request && response) {
            if (typeof request.url === 'string' && response.writeHead) {
                ...
                if (regData.hasOwnProperty(urlPath)) {
                    regData[urlPath](request, response);
                } else {
                    ...
                    if (arr.length > 1) {
                        ...
                        while (j >= 0) {
                            ...
                            if (regData.hasOwnProperty(str)) {
                                params_FromTo.unshift(response);
                                params_FromTo.unshift(request);
                                regData[str].apply(this, params_FromTo);
                            } else {
    ...
    function ReadFile(response, path) {
        ...
        fs.readFile(path, function (err, regData) {
