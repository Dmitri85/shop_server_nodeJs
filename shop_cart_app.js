(function ($) {
    ...
    $.fn.shopcart = function (plugCart, options) {
        ...
        var shop = {
            ...
            linksCreator: function () {
                $.get('http://localhost:8888/askForCategories', function (data) {
                    var nav = $('#main-nav');
                    var link_content = '';
                    for (var cat in data) {
                        link_content = cat.toUpperCase();
                        $('<li><a href="/' + cat + '">' + link_content + '</a></li>').appendTo(nav);
                    }
                    shop.linkColor();
                });
            },
            ...
            linkColor: function () {
                if (window.location.pathname == '/') {
                    $('#cat-name').html('COMICS');
                } else {
                    $('#cat-name').html(window.location.pathname.toUpperCase().replace('/', ''));
                }

                if (window.location.pathname == '/') {
                    var currentLink = document.querySelector('[href="/comics"]');
                    currentLink.style.color = '#CA8D00';
                    currentLink.style.background = '#dce1e9';
                    currentLink.style.fontWeight = "800";
                } else {
                    var currentLink = document.querySelector('[href="' + window.location.pathname + '"' + "]");
                    currentLink.style.color = '#CA8D00';
                    currentLink.style.background = '#dce1e9';
                    currentLink.style.fontWeight = "800";
                }
            ...
            askForId: function () {
                if (localStorage.getItem('cartUserId')) {
                    $.get('http://localhost:8888/askCart/' + localStorage.getItem('cartUserId'), function (data) {
                        if (data) {
                            shop.appendProductsFromServerOnLoad(data);
                        }
                    });
                } else {
                    $.get('http://localhost:8888/askId', function (data, status) {
            ...
            saveCartToServer: function () {
                var productsOnCartJson = shop.collectCartToJson();
                $.get('serverCart/' + productsOnCartJson + '&' + localStorage.getItem('cartUserId'), function (data) {

                })
            },

            loopItemsFromCart: function (htmlCartRowProduct) {
                var obj = {};
                obj.name = htmlCartRowProduct.find(".crt_name").text();
                obj.qty = htmlCartRowProduct.find(".crt_qty input").val();
                obj.price = htmlCartRowProduct.find(".crt_price").text();
                return obj;
            },

            collectCartToJson: function () {
                var itemsArrCollector = [];
                $(plugCart).find(".card_product").each(function () {
                    var item = shop.loopItemsFromCart($(this))
                    itemsArrCollector.push(item);
                });
                return JSON.stringify(itemsArrCollector);
            },

            appendProductsFromServerOnLoad: function (product) {
                for (var i = 0; i < product.length; i++) {
                    var new_cart = $('<tr>', { class: 'card_product' }).data('details', product[i]);;
                    $('<td>', { class: 'delete' }).text('x').appendTo(new_cart).click(shop.deleteItem);
                    $('<td>', { class: 'crt_name' }).text(product[i].name).appendTo(new_cart);
                    var tdForInput = $('<td>', { class: 'crt_qty' }).appendTo(new_cart);
                    $('<input>', { type: 'number', value: product[i].qty }).appendTo(tdForInput);
                    $('<td>', { class: 'crt_price' }).text(parseFloat(product[i].price)).appendTo(new_cart);
            ...
            callFromServer: function () {
                ...
                if (loadingProcces == false) {
                    ...
                    if (window.location.pathname == '/') {
                        $.post(settings.products_load_url + "/" + loadQtyFirst + '/' + loadQtyLast, { from: loadQtyFirst, to: loadQtyLast, pathname: urlPath }, function (data) {
                            var products = data;
                        ...
                        $.post(window.location.pathname + "/" + loadQtyFirst + '/' + loadQtyLast, { from: loadQtyFirst, to: loadQtyLast, pathname: urlPath }, function (data) {
                            var products = data;
                            loadQtyFirst += settings.url_load_page_count;
                            loadQtyLast += settings.url_load_page_count;
                            shop.collectProducts(products);
